...
-- boss-level keys are "<delveName>##<bossIndex>".
local expandedDelve  = {}
local expandedBoss   = {}
...
local delveLowerName = {}
local delveLowerZone = {}
...
local function RefreshFilteredData()
    ...
    table_sort(filteredData, function(a, b)
        ...
        if sortField == "speed" then
            -- Unrankable delves (no time) always sink to the bottom regardless of direction.
...
E:RegisterModule(function()
    ...
    -- Forward-declared so row/header click handlers can call it.
    local Refresh
